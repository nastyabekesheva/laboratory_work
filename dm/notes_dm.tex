...
\begin{theorem}
	$$\forall a\in\Mm,\>\forall m,\>n\in \mathbb{N}_0:\>a^m\bullet a^n=a^{m+n}$$
	\begin{proof}
		MMI:\\$\forall m$ - фіксованe: $n=0$(база): $a^m\bullet a^0=a^m\bullet e=a^m=a^{m+0}\\n\to n+1\tab a^m\bullet a^{n+1}=a^m\bullet a^n\bullet a=a^{n+m}\bullet a=a^{m+n+1}$  
	\end{proof}
\end{theorem}
...
\begin{fact*}
	...
	\end{array}\begin{array}{l}
		\Mm=\langle g\rangle:\>g^k\mapsto k\\
		 \textrm{Нехай існує } \Mm=\langle g\rangle,\>|\Mm|<\infty\\\Rightarrow \exists \min g:\>g^s=g^m,\>m<s\\\langle2\rangle\in\mathbb{Z}_{20}\textrm{ за }\bullet\>\langle2\rangle=\{\overbrace{1,\>2}^{\textrm{передп.}},\overbrace{4,\>8,\>16,\>32}^{\textrm{цикл}}\}\sim C_{2,\>4}
	\end{array}$ 
\end{fact*}
\chapter{Лекція 2}
...
\begin{lemma}
	$$\langle G,\>\bullet\rangle,\>a\in G,\>\langle a\rangle\textrm{ - скінченна}\Rightarrow\langle a\rangle\textrm{ н містить передпорядку}$$ 
	\begin{proof}
		$\\\begin{array}{c}
			\includegraphics[width=3cm]{graph3}
		\end{array} \langle a\rangle$ - скінченна $\Rightarrow\begin{array}{l}
			\exists m,\>s:\>s>m>0.\\
			a^s=a^m\Rightarrow a^s\bullet(a^m)^{-1}=e,\>a^{s-m}=e.
		\end{array}$
	\end{proof}
\end{lemma}
...
\begin{lemma}
	...
	\end{align*}
	\begin{proof}
		$\\$\textcircled{$\Leftarrow$} $u=k\bullet n\Rightarrow g^u=(g^n)^k=e^k=e$$\\$\textcircled{$\Rightarrow$} Нехай $u\>\bar{\vdots}\>n\Rightarrow u=nq+r,\>0<r<n\\\tab e\bullet g^u=(g^n)^q\bullet g^r=e^q\bullet g^r=g^r\Rightarrow n$ - не порядок $g$ - Упс!
	\end{proof}
\end{lemma}
